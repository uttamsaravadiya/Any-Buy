        ...
        {product.map((deal) => (
          ...
          >
            <img
              src={deal.image}
              alt={deal.title}
              className="w-full h-40 object-contain hover:scale-110 transition-transform"
            />
            <p className="mt-2 text-center text-gray-800 font-medium">
              {deal.title}
            </p>
            {hoverCrad === deal.id && (
            <button className='bg-black text-white p-1 text-center rounded-full transform transition ease-in-out duration-300'>
              Buy Now
            </button>
          )}
          </div>
